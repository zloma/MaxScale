...
do
	...
	do
		...
		stop_cmd_var="$prefix"_"$num"_stop_db_command
                GRN=`expr $i + 1`
                eval 'export $start_cmd_var="mysqld_multi --defaults-file=${script_dir}/multiple_servers.cnf start $GRN"'
                eval 'export $stop_cmd_var="mysqld_multi --defaults-file=${script_dir}/multiple_servers.cnf  stop  $GRN"'

                start_cmd_var="$prefix"_"$num"_cleanup_db_command
                GRN=`expr $i + 1`
                eval 'export $start_cmd_var="${script_dir}/cleanup_db.sh $GRN"'
