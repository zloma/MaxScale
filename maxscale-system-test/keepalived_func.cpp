...
{
    MYSQL * keepalived_conn = open_conn(Test->maxscales->rwsplit_port[0], virtual_ip, Test->maxscales->user_name,
...
void configure_keepalived(TestConnections* Test, char * keepalived_file)
{
    ...
    Test->tprintf("Copying maxscale_000 IP %s\n", Test->maxscales->IP[0]);
    sprintf(client_ip, "%s", Test->maxscales->IP[0]);
    last_dot = client_ip;
    ...
    for (i = 0; i < Test->maxscales->N; i++)
    {
        ...
        Test->maxscales->ssh_node_f(i, true, "sed -i \"s/###virtual_ip###/%s/\" /etc/keepalived/keepalived.conf",
                                    virtual_ip);
